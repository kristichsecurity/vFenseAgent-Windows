	      ...
	      const char * passed_home_path, int id_offset, const char * disp_username)
{
  ...
  printf ("%s:unused_by_nt/2000/xp:%u:%u:%s%s%s%s%s%s%s%s:%s:/bin/switch\n",
	  envname,
	  uid + id_offset,
	  gid + id_offset,
	  envname,
...
main (int argc, char **argv)
{
  ...
  if (!isNT)
	...
	strcpy (passed_home_path, "/home/");
      if (!disp_username)
        {
	  printf ("admin:use_crypt:%lu:%lu:Administrator:%sadmin:/bin/switch\n", 
	  ...
	  printf ("%s:use_crypt:%lu:%lu:%s:%s%s:/bin/switch\n", 
		  disp_username,
		  uid,
		  DOMAIN_ALIAS_RID_ADMINS,
		  disp_username,
